...
<div class="bg-white p-4 rounded-lg shadow-md"> {{-- Reduced padding from p-6 to p-4 --}}
    <div class="flex justify-between items-center mb-4"> {{-- Reduced mb-6 to mb-4 --}}
        <h1 class="text-xl font-bold text-gray-800">Data Dosen</h1> {{-- Reduced text-2xl to text-xl --}}
    </div>
    ...
    <div id="myTabContent">
        <div class="p-3 rounded-lg bg-gray-300 dark:bg-gray-800" id="dosen" role="tabpanel" aria-labelledby="dosen-tab"> {{-- Reduced padding from p-4 to p-3 --}}
            <div class="flex justify-end mb-3"> {{-- Reduced mb-4 to mb-3 --}}
                <a href="{{ route('dosen.create') }}" class="bg-green-800 hover:bg-green-900 text-white px-3 py-1.5 rounded-md text-sm transition duration-200 ease-in-out">Tambah dosen</a> {{-- Reduced padding and added text-sm --}}
            </div>
            @if (empty($dosen) || empty($dosen['data'])) {{-- Check if 'data' key exists and is not empty --}}
                <p class="text-gray-600 text-sm">Tidak ada data dosen yang tersedia.</p> {{-- Added text-sm --}}
            @else
                    ...
                    <table class="min-w-full bg-white border border-blue-300 rounded-lg">
                        <thead>
                            <tr class="bg-blue-800 text-white uppercase text-xs leading-normal"> {{-- Reduced text-sm to text-xs --}}
                                <th class="py-2 px-4 text-left ">NIDN</th> {{-- Reduced padding from py-3 px-6 to py-2 px-4 --}}
                                <th class="py-2 px-4 text-left">Nama dosen</th> {{-- Reduced padding --}}
                        ...
                        <tbody class="text-gray-700 text-xs font-light"> {{-- Reduced text-sm to text-xs --}}
                            @foreach ($dosen['data'] as $d) {{-- Changed from $dosen to $dosen['data'] --}}
                                <tr class="border-b border-blue-900 hover:bg-blue-100">
                                    <td class="py-1.5 px-4 text-left whitespace-nowrap">{{ $d['nidn'] }}</td> {{-- Reduced padding to py-1.5 px-4 --}}
                                    <td class="py-1.5 px-4 text-left">{{ $d['nama_dosen'] }}</td> {{-- Reduced padding --}}
                                    <td class="py-1.5 px-4 text-left">{{ $d['program_studi'] }}</td> {{-- Reduced padding --}}
                                    <td class="py-1.5 px-4 text-left">{{ $d['email'] }}</td> {{-- Reduced padding --}}
                                    <td class="py-1.5 px-4 text-center"> {{-- Reduced padding --}}
                                        <div class="flex item-center justify-center space-x-1"> {{-- Reduced space-x-2 to space-x-1 --}}
                                            <a href="{{ route('dosen.show', $d['nidn']) }}" class="text-blue-500 hover:text-blue-700 transition duration-200 ease-in-out" title="Lihat Detail">
                                            ...
                                            <a href="{{ route('dosen.edit', $d['nidn']) }}" class="text-yellow-500 hover:text-yellow-700 transition duration-200 ease-in-out" title="Edit">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.status-option').forEach(option => {
            option.addEventListener('click', function(event) {
                ...
                // untuk memperbarui status dosen di database.
                // Anda akan membutuhkan nidn dosen (primary key) dan status yang dipilih.
                const nidn = this.closest('tr').querySelector('td:first-child').textContent.trim();
                console.log(`Mengirim update status untuk nidn: ${nidn} menjadi: ${selectedStatus}`);

                // Contoh AJAX request (Anda perlu mengimplementasikannya sesuai API Anda)
                // fetch(`/api/dosen/${nidn}/status`, { // Ganti dengan endpoint API Anda
                //     method: 'PUT',
                //     headers: {
